...
"""class SearchableMixin(object):
    #for cls read class, these methods are being called on classes, not objects
    @classmethod
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    def followed_posts(self):
        followed = Post.query.join(
            followers, (followers.c.followed_id == Post.user_id)).filter( #queries db for all posts by followed users
                followers.c.follower_id == self.id)
        own = Post.query.filter_by(user_id=self.id) #queries db for user's own posts
        return followed.union(own).order_by(Post.timestamp.desc())
    
    def is_liked(self, post):
        return self.liked_posts.filter(
            likes.c.liker_id == likes.c.user.id).count() > 0
    ...
    def new_messages(self):
        last_read_time = self.last_message_read_time or datetime(1900, 1, 1) #checks to see messages received since last check 
...
class Post(
    #SearchableMixin ,
    db.Model):
    #__searchable__ = ['body'] Uncomment these lines to implement search later
    id = db.Column(db.Integer, primary_key=True)
